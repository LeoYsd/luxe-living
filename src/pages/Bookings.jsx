...
import { Calendar, MapPin, Users, Clock, ExternalLink, Compass, X, AlertCircle } from "lucide-react";
...
export default function BookingsPage() {
  ...
  const categorizeBookings = () => {
    ...
    bookings.forEach(booking => {
      if (booking.status === 'cancelled') {
        return; // Skip cancelled bookings in categorization
      }
  ...
  const canCancelBooking = (booking) => {
    ...
    // 2. Check-in date is at least 24 hours away
    const checkIn = parseISO(booking.check_in);
  ...
  const { upcoming, current, past } = categorizeBookings();
              ...
              <AlertDialogDescription>
                ...
                {selectedBooking && (
                  <div className="mt-4 p-3 bg-slate-50 rounded-lg text-sm text-slate-700">
                    ...
                    <p><strong>Total:</strong> ${selectedBooking.total_price}</p>
                  </div>
                )}
